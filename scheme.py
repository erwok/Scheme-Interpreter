...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_numberp(expressions.rest.first):
            targetVal = expressions.rest.first
        else:
            targetVal = scheme_eval(expressions.rest.first, env)
        env.bindings[target] = targetVal
        return target
        # END PROBLEM 5
